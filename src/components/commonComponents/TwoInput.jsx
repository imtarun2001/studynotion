...
const TwoInput = ({firstInputFieldData,secondInputFieldData,onChange}) => {
    ...
    <div className='flex w-full justify-between items-center'>
        {
            arr.map((ele,index) => (
                <div key={index} className='flex w-[45%] relative flex-col gap-[0.3rem] justify-start items-start'>
                    ...
                    <input type={ele.type} name={ele.name} id={ele.id} placeholder={ele.placeholder} onChange={onChange} value={ele.value} className='bg-gray-800 z-5 border-b-[1px] border-gray-400 w-full p-[0.7rem] rounded-md placeholder-gray-600'/>
                    {
